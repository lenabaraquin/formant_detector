...
class Formants:
    def __init__(
        self, file_path: str, frame_duration: float = 0.1, cutoff_quefrency: int = 35
    ) -> None:
    ...
    def _extract_spectral_envelope(self, analyzed_waveform):
        ...
        liftred_cepstrum = [
            0 if i > self.cutoff_quefrency else x for i, x in enumerate(cepstrum)
        ]
    ...
    def _framing(self) -> list:
        ...
        self.middle_frame = self.sound_waveform[
            len(self.sound_waveform) // 2 : len(self.sound_waveform) // 2 + frame_size
        ]
        return [
            self.sound_waveform[i : i + frame_size]
            for i in range(0, len(self.sound_waveform), frame_size)
        ]
    ...
    def _extract_formants(spectral_envelope: list) -> list:
        """compute formants from the spectral envelope of a waveform"""
        formants = []
        for i in range(1, len(spectral_envelope) - 1):
            if (
                spectral_envelope[i] > spectral_envelope[i - 1]
                and spectral_envelope[i] >= spectral_envelope[i + 1]
            ):
                formants.append(i)
        return formants
...
iii = Formants(iii_file_path, 0.02, 38)
uuu = Formants(uuu_file_path, 0.02, 49)
...
plt.plot(iii.formants[0], iii.formants[1], "o")
plt.show()
